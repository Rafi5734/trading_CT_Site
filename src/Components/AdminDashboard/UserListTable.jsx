...
const UserListTable = ({
  ...
  const handleDelete = (userEmail, userName) => {
    ...
    setModalContent(
      <DeleteUser refetch={refetch} setModalOpen={setSmallModalOpen} userEmail={userEmail} userName={userName} endpoint={'admin/delete-user'} />
    );
  };
                      ...
                      {tbody?.payment_status === "paid" || tbody?.payment_status === "Paid" ? (
                        ...
                        <>
                          <div className="text-left font-montserrat text-base font-normal">
                            ...
                            <p className="text-[#f44e4e] capitalize text-center text-base rounded-full bg-red-500 bg-opacity-20 w-24 font-bold">
                              {tbody?.payment_status}
                            </p>
                          </div>
                        </>
                      ...
                      <Menu>
                        ...
                        <Menu.Items className="absolute left-[-147px] z-[10] flex min-w-[170px] flex-col bg-primary rounded-xl border border-slate-500">
                          ...
                          <Menu.Item>
                            {({ active }) => (
                              <Link
                              onClick={()=>handleDelete(tbody?.email, tbody?.name)}
                                className={`mx-auto block w-full rounded-md p-[10px] text-center font-inter text-[16px] font-normal hover:bg-[#6c8c99] ${
                                  active ? "bg-[#6c8c99]" : ""
                                }`}
                              >
