...
import DepositContent from "./Content/DepositContent";
import { AiOutlineSearch } from "react-icons/ai";
import { Menu } from "@headlessui/react";
import { Link } from "react-router-dom";
import SmallModal from "../../SharedComponent/Modal/SmallModal";
import DepositDelete from "../../SharedComponent/Modal/ModalContent/DepositDelete";
import PhotoModalContent from "./Content/PhotoModalContent";
import PhotoModal from "../../SharedComponent/Modal/PhotoModal";

const DepositListTable = ({ pageTitle, changeFont, tableHead, tableData }) => {
  ...
  const handleDelete = (id, name) => {
    ...
    setModalContent(
      <DepositDelete setModalOpen={setSmallModalOpen} id={id} name={name} />
    );
  };
  ...
  const handleSeePhoto = (photoUrl) => {
    setModalTitle("Deposit Payment History");
    setModalContent(<PhotoModalContent photoUrl={photoUrl} />);
    setPhotoModal(true);
  };
  return (
    <div>
                ...
                <>
                  {currentItems?.map((tbody, i) => {
                    ...
                    return (
                      <tr
                        key={i}
                        className="h-[80px] text-sm leading-none text-[#edecec]"
                      >
                        <td className="px-2">
                          <div className="text-center font-montserrat text-base font-normal">
                        ...
                        <td className="px-2">
                          <div className="text-left overflow-x-scroll pb-1 max-w-[200px] font-montserrat text-base font-normal">
                            <p>{tbody?.depositAddress}</p>
                          </div>
                        </td>
                        <td className="px-2">
                          <div className="text-left font-montserrat text-base font-normal">
                        ...
                        <td className="px-2">
                          <div className="text-left font-montserrat text-base font-normal">
                            {tbody?.imageUrl ? (
                              <button
                                onClick={() => handleSeePhoto(tbody?.imageUrl)}
                                className="bg-[#111827] hover:bg-primary hover:text-[#111827] hover:font-bold duration-200 px-1 min-w-[100px] font-montserrat uppercase text-[12px] rounded-full border-2 border-primary"
                              >
                                See Photo
                              </button>
                            ) : (
                              <button
                                onClick={() => handleSeePhoto(tbody?.imageUrl)}
                                className="bg-[#111827] px-1 min-w-[100px] font-montserrat uppercase text-[12px] rounded-full border-2 border-primary"
                              >
                                No photo
                              </button>
                            )}
                        ...
                        {tbody?.status === "approved" ? (
                          ...
                          <td className="px-2">
                            <p className="text-[#f44e4e] text-center text-sm rounded-full bg-red-500 bg-opacity-20 capitalize w-full font-montserrat font-medium">
                              {tbody?.status}
                            </p>
                          </td>
                        )}
                        <td className="relative pl-5">
                          <Menu>
                            ...
                            <Menu.Items className="absolute left-[-136px] z-[10] flex min-w-[170px] flex-col bg-primary rounded-lg">
                              <Menu.Item
                                disabled={
                                  tbody?.status === "approved" ? true : false
                                }
                              >
                                {({ active }) => (
                                  <Link
                                    ...
                                    className={`mx-auto ${
                                      tbody?.status === "approved"
                                        ...
                                        : "hover:bg-[#2e3b56]"
                                    } block w-full rounded-lg p-[10px] text-center font-inter text-[16px] font-normal  ${
                                      active ? "bg-[#6c8c99]" : ""
                                    }`}
                                  >
                                    Edit Status
                              ...
                              <Menu.Item>
                                {({ active }) => (
                                  <Link
                                    ...
                                    className={`mx-auto block w-full rounded-lg p-[10px] text-center font-inter text-[16px] font-normal hover:bg-[#2e3b56] ${
                                      active ? "bg-[#6c8c99]" : ""
                                    }`}
                                  >
                                    Delete
